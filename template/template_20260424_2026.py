...
6. **含蓄表达**：当主持人反复强调风险、泡沫、大户离场、估值过高，且没有给出相反的方向性理由时，即使没有直接说“卖出”，也往往意味着看空（open_sell），因为他认为后市偏弱、应离场或应该做空。反之亦然。但要结合第5条区分是“明确看空方向”还是“仅仅否定当前买入动作”；后者应判为 close_buy。
...
- **open_sell**：与 open_buy 相反；主持人对该标的给出偏空、偏卖出、偏做空的建议，或给出能支撑做空的理由与判断。
  - 常见偏空证据：
    ...
    - **市场情绪**：泡沫迹象、投机过度、散户追高。
  - 注意：即使主持人没有直接说“卖出”，但若通过上述证据明确暗示市场将下跌、应离场、应卖出或应做空，也应判为 open_sell。
  - 但若主持人只是表达“先别买”“不要追”“观望一下”“等拉回再买”，这属于否定当前做多动作，应优先判为 **close_buy**，而不是 **open_sell**。
...
背景补充（非常重要）：
- 输入中除了 Transcript 之外，还可能提供 Helper。当前 Helper 可能包含从节目投影片/画面中抽取出来的 OCR 文字（例如 `ocr_text`）。
- Helper 中的 OCR 文字用途是辅助修正 ASR、补充投影片上清楚出现但口播含糊的实体、以及帮助辨认台湾财经语境中的专有名词。
- Helper 中的 OCR 可能存在识别错误、断行错误、残缺片段、重复片段、繁简混杂（在进入模型前通常已尽量转为简体）等问题，因此不能把它当成绝对真值。
- 信任顺序必须是：Transcript 第一，Helper 中的 OCR 第二。
- 当 Transcript 与 Helper 中的 OCR 一致时，可把 OCR 视为强化证据；当 Transcript 含糊、疑似 ASR 错字/近音词时，可用 OCR 协助修正；但若 Transcript 已经清楚表达，不能因为 OCR 某个可疑片段就强行推翻 Transcript。
- 当 Transcript 与 Helper 中的 OCR 不一致时，优先遵守 Transcript；只有在 OCR 明显是在补足一个显而易见的 ASR 缺口、或帮助澄清一个原本就高度歧义的短词/专有名词时，才可吸收 OCR 信息做修正。
- Transcript 的主持人与常见来宾主要是台湾财经语境，口语中可能带有台湾国语、台语/闽南语习惯、以及台湾财经圈常见俗称。
- 因此，在处理 ASR、谐音、错字、近音词时，不能只按“标准普通话”思考，也要考虑台湾口音与闽南语影响下的发音漂移。
- 要特别警惕前后鼻音的漂移，例如 `xing -> xin`、`bing -> bin` 这类只差尾音 `-ng / -n` 的 ASR 偏移；遇到这类情况时，应优先判断它是不是同一个真实实体名被轻微听错，而不是立刻拆成多个标的或跳到完全不同的公司。
- 对于短中文公司名、集团名、产业名，若出现一字之差的可疑词，应优先考虑：
  1) 台湾语境下更自然、更常见的市场说法；
  2) 台湾口音 / 台语 / 闽南语影响下更接近的发音路径；
  3) 结合上下文后更高频、更合理的金融对象；
  而不是只做字面最接近的公司名匹配。
- 例如：若原文出现“联发可”“联发可集团”这类可疑词，在台湾财经语境下应优先怀疑是否为“联发科”“联发科集团”之类受口音/ASR影响的表达；不得仅因个别字形或近音，就草率跳到另一家完全不同的实体。
- 对于台湾财经口播里常见的“短中文公司名连续罗列”，要特别警惕 ASR 把多个相邻标的黏成一个长串。判断顺序必须是：
  1) 先问：整段长串本身是否高置信对应一个真实、自然、常见的单一上市公司/集团名？
  2) 若是，则保留整段作为一个 instrument，并仅在标准化字段里修正（例如某个三字/四字集团名整体高置信只是在单字上发生 ASR 漂移时，可保留整段 instrument，再在标准化字段修正到真实实体）。
  3) 若否，而且该长串明显可拆成 2 个或以上彼此独立、在台湾财经语境中常见的短公司名/集团名，则必须拆分成多个 instrument；不得把这种不自然的黏连长串整体保留。
  4) 也就是说：宁可把“不像真实实体名的黏连串”拆开，也不要把它整体当成一个新公司名。
- 高优先拆分线索（很重要）：
  - 某个长串里包含两个都像独立公司简称/集团名的片段，且整串本身不像自然公司全名；
  - OCR 对该处没有提供清晰单一公司名支持，而 Transcript 又是明显口播列举语气。
- 具体示例（高优先级）：
  - 若 Transcript 出现“国乔东联还会怕景气反转吗”，优先理解为并列列举，拆成“国乔”和“东联”；不要保留“国乔东联”这个整体。
  - 若 Transcript 出现“亚泥远纺还会怕外销压力吗”，而整串本身不像自然单一实体名，则优先拆成“亚泥”和“远纺”；不要把“亚泥远纺”整体保留。
  - 若 Transcript 出现某个整体上很像真实单一实体、只是在其中一字或一音上发生漂移的集团名，则先判断它是否高置信指向一个常见单一实体；若高置信，则可保留整串 instrument 并在标准化字段修正；只有在“整串不是单一实体”时，才拆分。

因子 ASR 修正提示（高优先级词表）：
- 主持人节目中常见的因子词，优先按下列标准概念理解：
  - 動能因子
  - 波動因子
  - 質量因子
  - 股息因子
  ...
  - 市值因子
  - 流動性因子
- 若 Transcript 出现与上述词表高度近音、近形、疑似 ASR 漂移的写法，应优先尝试修正到这些标准因子词，而不是直接判为“不明”或丢弃。
- 例如：若出现“鼓励因子”这类可疑词，在明确财经/因子语境下，应优先怀疑是否为“股利因子/股息因子”等常见因子词；若上下文不足以确定具体哪一个，也应尽量保留为潜在因子概念，而不要轻易不抽取。

目标：
从 Transcript 中抽取所有「可交易的金融标的」名称，既包括【显式提及】也包括【隐含提及】。
若 Helper 中提供 OCR 文本，应把它作为辅助上下文一起使用，但最终抽取结果仍需以 Transcript 为主、以 OCR 为辅。

优先级（按顺序理解）：
1) 先结合 Transcript 与 Helper 中的 OCR，修正高置信的 ASR / 谐音 / 错字问题；
2) 再尽量完整抽取所有潜在可交易标的；
3) 对每个抽取结果补充 geography（国家 / 区域暴露），用于下游分类与统计。

核心定义：
可交易的金融标的 = 可在金融市场上直接买卖的具体金融工具（股票/指数/外汇/商品/期货合约/具体债券/主流加密货币），以及在财经语境中可自然对应为可交易市场暴露的广义对象（如“美国股市”“A股”“半导体板块”“价值因子”）。
范围声明：本任务不包含 ETF/基金类产品（即使可交易也不抽取）。

=== 字段约束（必须严格遵守）===
1) instrument（原文标的）：
   - 必须来自 Transcript 的连续子串（精确抄写），不得改写/翻译/补全。
   - 不得包含前后空格；只写标的名称本身（必要时可从更长黏连片段中裁剪出一个或多个标的名称，但每个 instrument 仍需各自是连续子串）。
   - 目标：最大化保留 Transcript 原文表述，用于可追溯、可回定位。
2) instrument_normalized（主标准化标的，英文/主键）：
   - 这是给当前下游流程使用的主键字段。
   - 默认应优先使用英文、ticker、官方英文简称或最常见英文市场名称。
   - 若该对象本来更适合用英文/ticker表达（如 S&P 500、BTC、AAPL、US 10Y Treasury），应直接使用该写法。
   - 对个股/公司，默认优先使用发行主体原属市场或最主流本地上市身份，不要因为存在 ADR / 美股存托凭证代码，就默认改写成 ADR 版本。
   - 只有当 Transcript 原文明确提到 ADR / DR / 美股代码 / 纽交所 / 纳斯达克 / 美股上市语境时，才可把 instrument_normalized 写成对应 ADR / 美股版本；否则应优先保留本地/原属市场身份。
3) instrument_normalized_zh（中文标准化标的）：
   - 应给出最自然、最稳定、最适合中文财经语境检索的中文标准化名称。
   - 若原文本身已经是最合适的中文标准名称，可直接等于 instrument。
4) 标准化规则（同时适用于 instrument_normalized / instrument_normalized_zh）：
   ...
   - 处理 ASR 时，不仅要考虑标准普通话，也要考虑台湾国语、台语/闽南语影响下的近音。优先采用在台湾财经语境中更自然、更常见、且与上下文更一致的修正路径。
   - 对于短中文公司名/集团名，若仅凭单个字面相似就会把实体改到另一家公司，则必须非常保守；若上下文不足，不要因为松散字形相似就强行跳到某个具体 ticker。
   - 若无法可靠标准化，则 instrument_normalized / instrument_normalized_zh 必须等于 instrument（原样拷贝）。
   - 重要：原文只放在 instrument；标准化结果放在 instrument_normalized / instrument_normalized_zh。
   - 对于广义市场/行业/因子/地区暴露（如“美国股市”“A股”“半导体板块”“价值因子”），第一步默认保持概念本身，不要擅自改成某个具体指数或产品名称；这类对象留给下游分类步骤处理。
3) geography（国家/区域暴露）：
   - 必须输出国家代码（ISO3）或区域代码。
   - 允许的常见区域代码包括：GLOBAL / UNCLEAR / DEVELOPED / EMERGING / FRONTIER / LATAM / EUROPE / HKG / ASIAPAC / ASIAPACEXJP / ASIAPACDEV / ASIAPACEM。
   - geography 表示该标的或该广义市场概念的主要国家/区域暴露，而不是说话语言，也不一定等于上市地。
   - 若无法可靠判断，则填 "UNCLEAR"。
...
行业/因子标准化规则：
- 原则：对行业/因子相关的隐含标的，第一步应尽量保留 Transcript 原始概念，不要提前品牌化、指数化或产品化。
- 一般行业/因子概念应保持原样：
  - instrument = Transcript 原文提法（连续子串，原样保留）
  - instrument_normalized = instrument
- 这些一般概念应留给下游分类步骤去处理，不要在第一步把泛概念强行改写成某个具体指数名称。
- 第一阶段主要只处理高置信的 ASR / 谐音 / 错字修正；若不是明显的转录错误，就保持原始概念，不要擅自改造成具体产品或指数。
- 对于因子词，优先使用上文“因子 ASR 修正提示”中的高频词表做修正；若仍不够确定，则保留为原始因子概念，不要直接丢弃。
- 只有当 Transcript 原文本身已经明确出现具体指数名称时，才按原文明示内容做正常标准化；否则不得凭空补成具体指数名称。

3 地区/国家暴露：如“美国股市”“A股”“港股”“欧洲股市”“亚洲股市”等，可视为隐含标的候选。
- 对这类广义市场概念，第一步应优先保留原始概念：
  - instrument = Transcript 原文提法
  - instrument_normalized = instrument
- 不要擅自把“美国股市”改成“标普500”，也不要把“A股”改成“上证指数”；具体指数化留给下游处理。
- 注意：单独出现“股市”若无任何地域限定，一律视为泛指，不抽取。

=== geography 规则（第一步抽取用）===
1) 商品、加密货币、单一货币、货币对：默认 geography="GLOBAL"。
2) 债券 / 主权利率：若原文明确国家或主权归属（如“美债”“德债”“日债”）→ 对应 ISO3；否则 "GLOBAL"。
3) 个股 / 公司：
   - 若公司或股票明显对应单一国家/主要市场 → 填该 ISO3；
   - 若只看到模糊公司名且无法可靠判断 → "UNCLEAR"。
4) 指数 / 市场基准 / 广义市场：
   - 单一国家市场 → ISO3（如 美国股市→USA，A股/大陆股市→CHN，日股→JPN）
   - 香港市场 → HKG
   - 欧洲/拉美/亚太/新兴市场/发达市场等 → 对应区域代码
   - 全球宽基 → GLOBAL
   - 无明确地域且无法可靠判断 → UNCLEAR
5) 行业 / 板块 / 因子：
   - 若原文带有明确国家或区域限定（如“美国科技股”“中国半导体”“欧洲银行股”）→ 填对应 ISO3 / 区域代码；
   - 若没有明确限定 → "GLOBAL"
6) 不得仅因用中文表达，就把 geography 默认判为 CHN；例如“电力”“半导体”“消费”“银行”在无国家限定时都应默认 "GLOBAL"。
...
4) 媒体 / 报纸 / 网站 / 栏目 / 出版物 / 新闻来源本身不是可交易标的：如“日经中文”“彭博社”“华尔街日报”“路透”等一律不抽取，除非 Transcript 明确提到的是对应指数/上市公司，而不是媒体本身。
5) 人物、群体昵称、投资者原型、社会角色本身不是可交易标的：如“渡边太太”“巴菲特”“散户”“外资”“财长”等一律不抽取。
6) 协议、条约、政策口号、宏观叙事、事件标签本身不是可交易标的：如“广场协议”“广场协议2.0”“关税战”“去杠杆”“MEGA”等一律不抽取；只有当 Transcript 同时明确点出其对应的可交易资产时，才抽取那个资产，而不是抽取这些概念标签本身。
...
=== OCR 使用规则（新增，必须遵守）===
1) OCR 是辅助证据，不是主证据；默认优先相信 Transcript。
2) OCR 最适合用于：
   - 修正 ASR 弄错的公司名 / 集团名 / 市场名 / 板块名 / 因子名；
   - 补足口播非常含糊、但投影片清楚展示的可交易对象；
   - 帮助辨认台湾财经语境中的简称、俗称、近音词。
3) OCR 不适合被直接当作绝对真值：
   - 若 OCR 本身像是错字、乱码、断裂短语、重复残句，不得仅凭 OCR 强行抽取新标的；
   - 若 OCR 与 Transcript 冲突，而 Transcript 已清楚，则按 Transcript 处理。
4) 若 OCR 提供了更完整、更标准的写法，而 Transcript 中有明显 ASR 漂移，可据此修正 instrument_normalized / instrument_normalized_zh；但 instrument 仍必须来自 Transcript 的连续子串，不能改抄 OCR 原文。
5) 若某标的只在 OCR 中出现、在 Transcript 中完全没有任何对应提法、暗示或相关语境，不要仅凭 OCR 强行抽取；只有当 OCR 明显是在补足 Transcript 中已出现但被 ASR 弄坏/弄糊的对象时，才可利用 OCR 帮助抽取。
6) 不得捏造 OCR 证据：
   - 只有当 Helper / OCR 中确实出现了可清楚对应的实体字串、或高置信可对应的别名时，才能把某次修正归因于 OCR。
   - 若 OCR 中没有该实体，或只有宏观图表/无关文字/模糊残句，不得声称“OCR 显示 / OCR 提到 / OCR 支持”该实体。
   - 若修正其实主要来自 Transcript 语境、台湾财经语境、近音判断或常识，就应按这些理由处理，不得把理由伪装成 OCR 证据。

=== 执行规则 ===
1) 完全匹配原文：instrument 必须是 Transcript 连续子串，保持原格式（不改写）。
   - 这是最高优先级硬规则，优先于所有 ASR 修正、OCR 修正、常识修正与标准化冲动。
   - 不得把修正后的标准名称写回 instrument；修正只能发生在 instrument_normalized / instrument_normalized_zh。
2) 去重规则（修改后）：
...
3) 拆分：相邻多个标的必须拆成独立条目。
   - 若某个长串明显是 ASR 把多个相邻短中文公司名/集团名黏在一起，必须拆开，不得把不自然的黏连串整体当成单一标的。
   - 只有当整段长串本身高置信对应一个真实、常见、自然的单一实体名时，才允许不拆。
   - 但拆分后的每个 instrument 仍必须各自能直接从 Transcript 裁剪为连续子串；若某次“拆分”必须靠替换字、补字、改字（例如把“刚”改成“钢”、把“硕”改成“塑”）才能得到子项，则这些修正后文字不得写入 instrument。
   - 若无法在不改字的前提下拆成多个有效 instrument，则不得发明新的 instrument 子串；此时只能：
     1) 保留原始较长子串作为 instrument，并仅在标准化字段中谨慎修正；或
     2) 若连整体都过于不可信，则宁可不抽取，也不要捏造新的 instrument。
   - 例：“国乔东联”应拆成“国乔”与“东联”；“亚泥远纺”应优先拆成“亚泥”与“远纺”；“华兴科”若高置信其实指“华新科”，则应整体保留并只在标准化字段修正。
...
field_instrument_normalized_deepseek = Field( ...,
    description=(
        ...
        "处理 ASR 时，不仅要考虑标准普通话，也要考虑台湾国语、台语/闽南语影响下的近音；应优先采用在台湾财经语境中更自然、更常见、且与上下文更一致的修正路径。"
        "对于短中文公司名/集团名，若仅凭单个字面相似就会把实体改到另一家公司，则必须非常保守；若上下文不足，不要因为松散字形相似就强行跳到某个具体 ticker。"
        "本字段允许比 instrument 更完整、更标准、更纠错，因为它承载的是你对原文想指什么的理解；但这种理解不得反向改写 instrument。"
        "本字段不得为空：若无法可靠标准化，则 instrument_normalized 必须等于原文 instrument（原样拷贝）。"
        "目标：输出最具代表性的、可交易的标准化资产。"
    ))

field_instrument_normalized_zh_deepseek = Field(
    ...,
    description=(
        "中文标准化标的名称。应给出最自然、最稳定、最适合中文财经语境检索的中文名称。"
        "若原文本身已经是最合适的中文标准名称，可直接等于 instrument。"
        "它应与 instrument_normalized 在语义上对应，但不要求字符串一致。"
        "通常当 instrument_normalized 使用英文、ticker 或英文市场名时，本字段应用对应的中文标准名。"
    ),
)

field_geography_extract_deepseek = Field(
    ...,
    min_length=1,
    description=(
        "该标的或该广义市场概念的国家/区域暴露代码。"
        "使用 ISO3（如 USA, CHN, JPN, TWN, HKG）或区域代码（如 GLOBAL, UNCLEAR, ASIAPAC, EUROPE, LATAM, EMERGING, DEVELOPED, FRONTIER, ASIAPACEXJP, ASIAPACDEV, ASIAPACEM）。"
        "该字段表示金融暴露地域，不等于说话语言，也不一定等于上市地。"
        "若无法可靠判断，则输出 UNCLEAR。GLOBAL 仅用于明确全球/跨区域/地域中性的情形。"
        "单一货币、货币对、商品、主流加密货币通常使用 GLOBAL；明确主权债、国家市场、国家公司则填对应 ISO3。"
    ),
)
...
class TradingInstrumentBase(BaseModel):
    ...
    instrument: str = Field(..., min_length=1,
        description=(
            "必须来自 Transcript 的连续子串（精确抄写），不得改写/翻译/补字/换字/补全；不得包含前后空格；"
            "只包含标的名称本身。"
            "必要时可从更长黏连片段中裁剪出标的名称，但裁剪后的每一项仍必须是 Transcript 中真实出现过的连续子串。"
            "instrument 只负责保留原文表面串，不负责表达你更完整、更标准、更纠错后的理解；"
            "任何 ASR 修正、标准化、金融化解释，都只能写在 instrument_normalized / instrument_normalized_zh，不能回写到 instrument。"
        ),
    )
    instrument_normalized: str = field_instrument_normalized_deepseek
    instrument_normalized_zh: str = field_instrument_normalized_zh_deepseek
    geography: str = field_geography_extract_deepseek
